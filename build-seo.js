 ...
 *
 * Each per-route file reuses the shell but injects:
 *   - <base href="/">  so relative asset paths (archspex-logo.png, ax2.js,
 *     shims, etc.) resolve from site root even when the page loads from a
 *     nested URL like /products/
 *   - unique <title>, <meta name="description">, self-referencing canonical,
 *     Open Graph / Twitter tags for SEO
 *   - window._initialRoute bootstrap so ax2.js paints the correct page
 *     immediately (no home-page flash)
 *
 * Additionally, for the Products and Manufacturers routes, the script fetches
 * a small snapshot from Supabase at build time and injects it as hidden
 * crawlable content — Googlebot with JavaScript disabled still sees real
 * product/brand names and can index them.
 *
 * Zero npm dependencies — uses only Node built-ins (fs, path, native fetch).
 */
...
function stripExistingSeo(html) {
  return html
    ...
    // Strip existing <base> tags — we'll inject our own
    .replace(/<base\s[^>]*>/gi, '');
}
...
// Inject <base href="/"> as the FIRST tag in <head>. Without this, relative
// asset paths (archspex-logo.png, ax2.js, prod_shim.js, image URLs, etc.)
// resolve relative to the current URL folder — so /products/ tries to load
// /products/archspex-logo.png which doesn't exist.
//
// <base> must come BEFORE any relative-href resource on the page for browsers
// to apply it to those resources. Putting it first inside <head> satisfies that.
...
function injectSsrContent(html, ssrHtml) {
  ...
  // Hidden container: off-screen positioning keeps it crawlable but invisible.
  // Avoids display:none which some crawlers discount.
  const wrapped =
...
function rebuildRoot(shell) {
  let html = stripExistingSeo(shell);
  html = injectBase(html); // safe at root too; makes behaviour consistent
...
async function writeRoute(route, shell) {
  ...
  let html = stripExistingSeo(shell);
  html = injectBase(html); // critical for nested-URL asset resolution
...
async function main() {
  ...
  console.log('[seo] Rewrote root index.html with home SEO meta + <base href="/">');
