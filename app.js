...
// ── PRODUCT CARD ──────────────────────────────────────────────────────────────
function prodCard(p){
  const pid = p.id||p.db_id;
  const pidStr = JSON.stringify(String(pid));
  // Store full product data in global cache so modal can find it
  if(!window._prodCache) window._prodCache = {};
  window._prodCache[String(pid)] = p;
  return `<div class="prod-card" onclick="openProduct(${pidStr})">
    <div class="prod-img-wrap">
      <img src="${p.img||p.image_url||''}" alt="${p.name}" loading="lazy" onerror="this.src='https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=400'">
      <span class="prod-cat-tag">${p.cat||p.category||''}</span>
      <button class="prod-wish" onclick="event.stopPropagation();handleWish(${pidStr},this)">
        <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M6 10.5L1 5.5a3 3 0 115-4.5L6 1l.5-.5a3 3 0 115 4.5L6 10.5z" stroke="#6b7685" stroke-width="1.3"/></svg>
      </button>
    </div>
    <div class="prod-body">
      <div class="prod-brand-row">${p.brand||''} · ${p.country||''}</div>
      <div class="prod-name">${p.name||''}</div>
      <div class="prod-meta">${p.meta||''}</div>
      <div class="prod-foot">
        <button class="btn-sm-navy" onclick="event.stopPropagation();openReq(${pidStr})">Request Sample</button>
        <button class="btn-sm-outline" onclick="event.stopPropagation();openProduct(${pidStr})">Spec ↓</button>
      </div>
    </div>
  </div>`;
}
...
function handleWish(id, btn){
  ...
  btn.innerHTML = '<svg width=12 height=12 viewBox="0 0 12 12"><path d="M6 10.5L1 5.5a3 3 0 115-4.5L6 1l.5-.5a3 3 0 115 4.5L6 10.5z" fill="#ef4444"/></svg>';
  btn.style.opacity = '1';
}
...
async function openProduct(id){
  ...
  // Check cache first
  const p = (window._prodCache && window._prodCache[sid]) ||
            (liveProducts||[]).find(x=>String(x.id)===sid||String(x.db_id)===sid||'db_'+String(x.db_id)===sid);
  ...
  if(!p){
    ...
    modalRight.innerHTML='<div style="padding:40px;text-align:center">Loading…</div>';
    try {
      ...
      if(data && data[0]){
        ...
        renderProductModal({
          ...
          specs:(()=>{try{return JSON.parse(d.specs||'{}')}catch(e){return{}}})(),
          swatches:[]
        }, modalRight);
        return;
      }
    } catch(e){ console.error(e); }
    modalRight.innerHTML='<div style="padding:40px;text-align:center;color:#999">Could not load product.</div>';
    return;
  }

  modal.classList.add('open');
  document.body.style.overflow='hidden';
  renderProductModal(p, modalRight);
}

function renderProductModal(p, modalRight){
  ...
  document.getElementById('modalImg').onerror = function(){ this.src='https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=600'; };
  document.getElementById('modalThumbs').innerHTML = [img].map((s,i)=>`<div class="modal-thumb active" onclick="switchModalImg(this,'${s}')"><img src="${s}" alt=""></div>`).join('');

  const sw = (p.swatches||[]).map(c=>`<div class="sw" style="background:${c}" onclick="this.parentNode.querySelectorAll('.sw').forEach(s=>s.classList.remove('active'));this.classList.add('active')"></div>`).join('');
  const specsObj = typeof p.specs === 'object' ? p.specs : {};
  const specs = Object.entries(specsObj).map(([k,v])=>`<div class="spec-row"><div class="spec-k">${k.replace(/_/g,' ')}</div><div class="spec-v">${v}</div></div>`).join('');
  const pid = JSON.stringify(String(p.id||p.db_id));

  modalRight.innerHTML = `
    <div class="modal-badge">${cat}</div>
    <div class="modal-brand">${p.brand} · ${p.country}</div>
    <div class="modal-title">${p.name}</div>
    <div class="modal-origin">${meta}</div>
    <p class="modal-desc">${desc}</p>
    ${sw?`<div style="font-size:9px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.8px;margin-bottom:7px">Available finishes</div><div class="swatches-row">${sw}</div>`:''}
    ${specs?`<div class="spec-table">${specs}</div>`:''}
    <div class="docs-row">
      <div class="doc-item"><div class="doc-icon"><svg width="14" height="14" viewBox="0 0 14 14" fill="none"><rect x="2" y="1" width="10" height="12" rx="1" stroke="var(--navy)" stroke-width="1.3"/><line x1="4" y1="4.5" x2="10" y2="4.5" stroke="var(--navy)" stroke-width="1" opacity=".5"/><line x1="4" y1="6.5" x2="10" y2="6.5" stroke="var(--navy)" stroke-width="1" opacity=".5"/></svg></div><div><div class="doc-name">Technical Data Sheet</div><div class="doc-size">PDF</div></div><div class="doc-dl">Download ↓</div></div>
      <div class="doc-item"><div class="doc-icon"><svg width="14" height="14" viewBox="0 0 14 14" fill="none"><rect x="1" y="1" width="12" height="12" rx="1" stroke="var(--navy)" stroke-width="1.3"/></svg></div><div><div class="doc-name">BIM / CAD File</div><div class="doc-size">RVT · DWG</div></div><div class="doc-dl">Download ↓</div></div>
    </div>
    <div class="modal-actions">
      <button class="btn-full-navy" onclick="closeModal();openReq(${pid})">Request a Sample →</button>
      <button class="btn-full-outline">Contact Brand Representative</button>
    </div>`;
}
    ...
    el.innerHTML=hallData.map(b=>`
      <div class="booth" onclick="showBoothPopup('${b.n}','${b.c}')">
        ...
        <div class="booth-body">
          <svg width="22" height="22" viewBox="0 0 22 22" fill="none" style="opacity:.7"><rect x="2" y="4" width="18" height="2.5" rx=".5" stroke="rgba(201,168,76,.9)" stroke-width="1.2"/><rect x="2" y="8.5" width="18" height="2.5" rx=".5" stroke="rgba(201,168,76,.9)" stroke-width="1.2"/><rect x="2" y="13" width="18" height="2.5" rx=".5" stroke="rgba(201,168,76,.9)" stroke-width="1.2"/></svg>
