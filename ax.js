...
async function renderHome(){
  ...
  // Load real news for home page
  loadHomeNews();
  var homeGuidesGrid = document.getElementById('homeGuidesGrid');
...
function setLoggedIn(user){
  ...
  var topbarReg = document.getElementById('topbarReg');
  if(topbarLogin){ topbarLogin.textContent = 'My Dashboard'; topbarLogin.onclick = function(){ showPage('dashboard'); }; }
  if(topbarReg){ topbarReg.textContent = 'Sign Out'; topbarReg.onclick = doSignOut; }
  var navReg = document.getElementById('navRegBtn');
  if(navReg){ navReg.textContent = 'My Dashboard'; navReg.onclick = function(){ showPage('dashboard'); }; }
  var navSub = document.querySelector('.nav-subscribe');
  if(navSub){ navSub.textContent = 'My Dashboard'; navSub.onclick = function(){ showPage('dashboard'); }; }
}
...
function setLoggedOut(){
  ...
  if(topbarReg){ topbarReg.textContent = 'Register Free'; topbarReg.onclick = function(){ openRegModal('register'); }; }
  var navReg = document.getElementById('navRegBtn');
  if(navReg){ navReg.textContent = 'Register Free'; navReg.onclick = function(){ openRegModal('register'); }; }
}
...
var subtypeMap = {
  ...
  'Envelope':    ['Facade Systems','Curtain Wall Systems','Cladding Systems','External Wall Finishes','Roofing Systems','Waterproofing Systems','Insulation Systems','Windows & Glazing','Doors & Entrance Systems','Shading & Louvers'],
  ...
  'Systems':     ['HVAC Systems','Electrical Systems','Plumbing Systems','Fire Protection Systems','Security Systems','Building Automation','Smart Systems','Elevators & Escalators']
};
...
function buildFilterSidebar(prods){
  ...
  if(catsEl){
    ...
    cats.forEach(function(c){
      ...
      inp.style.cssText = 'width:14px;height:14px;accent-color:var(--navy);cursor:pointer;flex-shrink:0';
...
function applyAndRender(){
  ...
  if(titleEl) titleEl.textContent = activeFilters.cat==='all' ? 'All Products' : activeFilters.cat==='Furnishing' ? 'FF&E' : activeFilters.cat;
...
function sortProds(val){
  ...
  var f = liveProducts.slice();
  if(val==='az') f.sort(function(a,b){ return (a.name||'').localeCompare(b.name||''); });
  else if(val==='brand') f.sort(function(a,b){ return (a.brand||'').localeCompare(b.brand||''); });
  else if(val==='newest') f.sort(function(a,b){ return (b.db_id||0)-(a.db_id||0); });
  var grid = document.getElementById('allProdGrid');
  if(grid) grid.innerHTML = f.map(prodCard).join('');
}
