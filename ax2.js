...
// ── PRODUCT PAGE (full standalone view) ───────────────────────────────────────
async function openProduct(id){
  ...
  if(!p){
    // Show the product page immediately with a loading title, then fetch
    ...
    if(!p){ if(_pt) _pt.textContent = 'Could not load product.'; return; }
  }
...
function renderProductPage(p){
  ...
  // Main image + thumbnails (single image for now — expand when we have multiple)
  ...
  // Middle spec list — pull from p.specs with sensible fallbacks
