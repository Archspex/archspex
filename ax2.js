...
const SUPABASE_KEY = 'sb_publishable_3ELK2S24C3VNoOwvNLtSJQ_OafdocZy';
...
function showPage(page, pushHistory=true){
  ...
  // Page-aware topbar search placeholder
  var _navSearch = document.getElementById('navSearchInput');
  if(_navSearch){
    if(page === 'manufacturers') _navSearch.placeholder = 'Search manufacturers, countries or product types…';
    else _navSearch.placeholder = 'Search products, brands…';
  }
  ...
  if(!p){
    ...
    try {
      ...
      const res = await fetch('https://jmmmexoykswsophzmytg.supabase.co/rest/v1/products?id=eq.'+dbId+'&select=*', {
        headers: {
          'apikey': 'sb_publishable_3ELK2S24C3VNoOwvNLtSJQ_OafdocZy',
          'Authorization': 'Bearer sb_publishable_3ELK2S24C3VNoOwvNLtSJQ_OafdocZy'
        }
      });
...
(function _injectBrandCardV2CSS(){
  ...
  var css = ''
    ...
    + '.bcv2-hero-img{width:100%;height:100%;object-fit:cover;display:block;transition:transform .5s}'
    + '.brand-card-v2:hover .bcv2-hero-img{transform:scale(1.06)}'
    // ---- Bookmark button — matches .prod-wish exactly ----------
    + '.bcv2-icon-btn{position:absolute !important;top:8px !important;right:8px !important;width:30px !important;height:30px !important;border-radius:50% !important;background:#FFFFFF !important;border:none !important;display:flex !important;align-items:center !important;justify-content:center !important;cursor:pointer !important;box-shadow:0 2px 8px rgba(0,15,40,.14) !important;transition:transform .18s ease-out, box-shadow .18s ease-out;z-index:10 !important}'
    + '.bcv2-icon-btn svg{width:14px;height:14px;color:#003366;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}'
    + '.bcv2-bookmark{}'
    + '.bcv2-bookmark:hover{transform:scale(1.08);background:#faf8f4 !important}'
    + '.bcv2-bookmark:active{transform:scale(.95)}'
    + '.brand-card-v2:hover .bcv2-bookmark{box-shadow:0 4px 12px rgba(0,15,40,.22) !important}'
    + '.bcv2-bookmark.is-saved svg{fill:#003366;stroke:#003366}'
    // ---- Featured Brand badge (same size/shape as bookmark, hover-expand) ---
    + '.bcv2-feat{position:absolute;right:8px;top:46px;background:transparent;border:none;padding:0;border-radius:100px;display:flex;flex-direction:row-reverse;gap:0;align-items:center;cursor:pointer;overflow:hidden;z-index:9;transition:gap 300ms cubic-bezier(.22,1,.36,1),padding 300ms cubic-bezier(.22,1,.36,1),background 300ms cubic-bezier(.22,1,.36,1),box-shadow 300ms cubic-bezier(.22,1,.36,1)}'
    + '.bcv2-feat:hover{gap:6px;padding:0 0 0 8px;background:rgba(255,255,255,.96);box-shadow:0 4px 12px rgba(0,15,40,.14)}'
    + '.bcv2-feat-ico{width:30px;height:30px;border-radius:50%;background:#FFFFFF;color:#C9A84C;display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 2px 8px rgba(0,15,40,.14);transition:box-shadow 300ms cubic-bezier(.22,1,.36,1)}'
    + '.bcv2-feat:hover .bcv2-feat-ico{box-shadow:none}'
    + '.bcv2-feat-ico svg{width:14px;height:14px;display:block}'
    + '.bcv2-feat-text{max-width:0;overflow:hidden;opacity:0;transition:max-width 300ms cubic-bezier(.22,1,.36,1),opacity 150ms cubic-bezier(.4,0,.2,1) 50ms;font-size:9px;font-weight:800;color:#003366;text-transform:uppercase;letter-spacing:.5px;white-space:nowrap;line-height:1}'
    + '.bcv2-feat:hover .bcv2-feat-text{max-width:120px;opacity:1}'
    ...
    + '.bcv2-logo{width:68px !important;height:68px !important;background:#FFFFFF;border:1px solid #CBD5E1;border-radius:8px;box-shadow:0 2px 8px rgba(0,15,40,.14);display:flex;align-items:center;justify-content:center;overflow:hidden;flex-shrink:0;color:#003366;font-size:11px;font-weight:700;position:relative;top:-15px}'
    ...
    // ── Market availability — small text ────────────────────────
    + '.bcv2-mrk-label{font-size:9px;font-weight:800;color:#7A8496;letter-spacing:.9px;text-transform:uppercase;line-height:1.3;margin:0}'
    + '.bcv2-markets{display:flex !important;flex-wrap:nowrap !important;align-items:center;gap:0 !important;border:none !important;padding:0 !important;margin-top:4px}'
    + '.bcv2-market{display:inline-flex !important;align-items:center;justify-content:flex-start;font-size:11px !important;font-weight:700 !important;color:#0A1A2C !important;background:transparent !important;border:none !important;padding:0 !important;border-radius:0 !important;line-height:1.3 !important;white-space:nowrap !important;box-sizing:border-box;letter-spacing:.3px}'
    + '.bcv2-market + .bcv2-market{margin-left:6px;padding-left:6px !important;border-left:1px solid #E5E7EB !important}'
    + '.bcv2-market img,.bcv2-check{display:none !important}'
    + '.bcv2-market-name{margin:0}'
    // ── Stat tiles — compact ───────────────────────────────────
    ...
    + '.bcv2-stat-txt span{font-size:9px !important;font-weight:600 !important;color:#7A8496 !important;letter-spacing:0 !important;margin-top:0 !important;line-height:1.3 !important;white-space:nowrap !important;overflow:visible !important;text-overflow:clip !important}'
    // ── Actions — match .btn-sm-navy exactly (9px/700/7px pad/radius 7) ──
    + '.bcv2-actions{display:flex;gap:5px;padding-top:10px;border-top:1px solid #CBD5E1;margin-top:2px}'
    + '.bcv2-view,.bcv2-contact{flex:1;text-align:center;font-family:Manrope,sans-serif;font-size:9px;font-weight:700;border-radius:7px;padding:7px;cursor:pointer;letter-spacing:.3px;transition:background .2s, border-color .2s;border:none}'
    + '.bcv2-view{background:#003366;color:#FFFFFF}'
    + '.bcv2-view:hover{background:#001A3D}'
    + '.bcv2-contact{background:#FFFFFF;color:#003366;border:1.5px solid #CBD5E1;padding:5.5px}'
...
function _brandCardV2(b){
  ...
  var stats = [
    ...
    {n: 50 + ((h>>8)  % 150),    l:'Docs',      ico:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="8" y1="13" x2="16" y2="13"/><line x1="8" y1="17" x2="14" y2="17"/></svg>'},
  ...
  return '<article class="brand-card-v2" onclick="openBrandProfile('+bid+')">'
    ...
    +       '<div class="bcv2-country"><span>'+countryTC+'</span>'+ (flagBig ? flagBig : '') +'</div>'
    ...
    +   '</div>'
    +   '<div class="bcv2-meta">'
    +   '<div class="bcv2-divider bcv2-div-a"></div>'
    +   '<div class="bcv2-mrk-label">Market Availability</div>'
    +   '<div class="bcv2-markets">'+marketHTML+'</div>'
    +   '<div class="bcv2-divider bcv2-div-b"></div>'
    +   '<div class="bcv2-stats" onclick="event.stopPropagation()">'+statsHTML+'</div>'
...
// Kill the visible "flash" when a card is clicked to open a product/brand page.
// Before this: click → async fetch (~200ms) → showPage swap; during the wait
// the card's :hover state transitions OFF (translate/shadow fade over .25s),
// producing a jarring flash right before the new page appears.
// Fix: on mousedown, freeze the card's transitions AND transform to their
// current values so no animation runs while the page is being torn down.
document.addEventListener('mousedown', function(e){
  var card = e.target && e.target.closest && e.target.closest('.prod-card, .brand-card-v2');
  if(!card) return;
  card.style.transition = 'none';
  // Reset after a beat so future re-renders behave normally.
...
// ── SITE IMAGE MANAGER ────────────────────────────────────────────────────────
// Applies images configured in the Supabase `site_images` table.
// Admin can change any image from the admin panel — no code edits needed.
(function(){
  var CACHE_KEY = 'ax_site_images_v1';

  function applyImages(rows){
    if(!rows || !rows.length) return;
    rows.forEach(function(row){
      if(!row.url || !row.selector) return;
      var els;
      try { els = document.querySelectorAll(row.selector); }
      catch(e){ return; }                       // bad selector -> skip silently
      if(!els || !els.length) return;
      els.forEach(function(el){
        if(row.type === 'background'){
          el.style.backgroundImage = "url('" + row.url + "')";
        } else {
          if(el.tagName === 'IMG') el.src = row.url;
          else el.style.backgroundImage = "url('" + row.url + "')";
        }
      });
    });
  }

  // Paint instantly from cache so there's no visible flash on repeat visits
  try {
    var cached = sessionStorage.getItem(CACHE_KEY);
    if(cached) applyImages(JSON.parse(cached));
  } catch(e){}

  async function loadSiteImages(){
    try {
      var res = await sb.from('site_images').select('*').order('sort_order',{ascending:true});
      var rows = res.data || [];
      if(!rows.length) return;
      applyImages(rows);
      try { sessionStorage.setItem(CACHE_KEY, JSON.stringify(rows)); } catch(e){}
      // re-apply shortly after in case a section rendered late
      setTimeout(function(){ applyImages(rows); }, 900);
    } catch(e){}
  }
