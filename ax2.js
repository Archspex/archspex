...
async function submitReq(){
  ...
  const _addrParts = [allInputs[5]?.value || '', allInputs[6]?.value || '', allInputs[7]?.value || '', allInputs[8]?.value || ''];
...
function srClearForm(){
  ...
  if(wrap) wrap.style.display = 'none';
  srRenderProductChip();
  ['srFirst','srLast','srEmail','srPhone','srCompany','srAddrBuilding','srAddrStreet','srAddrCity','srAddrCountry','srNotes','srRole'].forEach(function(id){
    const el = document.getElementById(id); if(el) el.value='';
  });
...
async function submitSR(){
  ...
  const addr    = [_addrBld, _addrSt, _addrCi, _addrCo].map(function(x){return (x||'').trim();}).filter(Boolean).join(', ');
  const _addrValid = _addrSt.trim() && _addrCi.trim() && _addrCo.trim();
  ...
  window.openReq = async function(){
    ...
    if(currentUser){
      ...
      setTimeout(function(){
        ...
        // Country into the 4th address field (inputs[8]) if present
        if(inputs[8] && !inputs[8].value) inputs[8].value = p.country || '';
