...
// ── DISCOVER TABS (Featured / Trending Categories / Newest) ───────────────────
var TRENDING_CATS = [
...
function renderDiscoverTab(tab){
  ...
  if(tab === 'trending'){
    content.innerHTML = '<div class="trending-cats">' + TRENDING_CATS.map(function(c){
      return '<div class="trending-cat" onclick="filterCatGo(\''+c.cat+'\')">'
           + '<img src="'+c.img+'" alt="" loading="lazy" onerror="this.style.display=\'none\'">'
           + '<div class="trending-cat-info">'
           + '<div class="trending-cat-meta">'+c.meta+'</div>'
           + '<div class="trending-cat-name">'+c.name+'</div>'
           + '</div></div>';
    }).join('') + '</div>';
    return;
  }
  ...
  // 'featured' = default order (newest from DB query); shows all
...
function switchDiscoverTab(btn){
  ...
  if(tabs){
    tabs.querySelectorAll('.discover-tab').forEach(function(t){ t.classList.remove('active'); });
  }
...
// ── SUBCATEGORY PILLS (products page) ──────────────────────────────────────────
