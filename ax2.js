...
function renderDiscoverTab(tab){
  ...
  if(!c) return;
  if(tab==='trending'){
    c.innerHTML = '<div class="trending-cats">' + TRENDING_CATS.map(function(t){
      return '<div class="trending-cat" onclick="filterCatGo(\''+t.cat+'\')"><img src="'+t.img+'" alt="" loading="lazy"><div class="trending-cat-info"><div class="trending-cat-meta">'+t.cat+'</div><div class="trending-cat-name">'+t.name+'</div></div></div>';
    }).join('') + '</div>';
    return;
  }
  ...
  list = list.slice(0,8);
  if(!list.length){
    c.innerHTML = '<div class="prod-grid"><div style="grid-column:1/-1;text-align:center;padding:60px 20px"><div style="font-size:40px;margin-bottom:16px">\u{1F3D7}</div><div style="font-size:16px;font-weight:700;color:var(--text);margin-bottom:8px">Products coming soon</div><div style="font-size:13px;color:var(--muted)">Our team is onboarding brands. Check back soon.</div></div></div>';
    return;
  }
...
function renderProductModal(p){
  ...
  if(el){ var _bn=(p.brand||''); el.textContent=_bn.toUpperCase(); if(_bn){ if(!document.getElementById('bp-brandlink-style')){ var _st=document.createElement('style'); _st.id='bp-brandlink-style'; _st.textContent='#modalBrand.brand-link{cursor:pointer;border-radius:6px;padding:2px 8px;margin:-2px -8px;transition:background .15s,color .15s}#modalBrand.brand-link:hover{background:var(--navy);color:var(--gold)}'; document.head.appendChild(_st); } el.classList.add('brand-link'); el.title='View '+_bn+' profile'; el.onclick=function(){ openBrandByName(_bn); }; } else { el.classList.remove('brand-link'); el.onclick=null; } }
...
function renderSubcatPills(){
  ...
  wrap.innerHTML = subs.map(function(s){
    ...
    var esc = s.replace(/'/g,"\\'");
    return '<button class="subcat-pill'+active+'" onclick="toggleSubcatPill(\''+esc+'\')">'+s+'</button>';
  }).join('');
}
...
function navTop(page){
  ...
  if(typeof brandPageFilters!=='undefined') brandPageFilters = { cat:'all', cert:[] };
...
async function renderManufacturers(){
  ...
  if(btitleEl) btitleEl.textContent = brands.length + ' brand' + (brands.length !== 1 ? 's' : '');

  // Also build country filters from data
  var countryEl = document.getElementById('brand-country-filters');
  if(countryEl && brands.length){
    var countries = [...new Set(brands.map(b => b.country).filter(Boolean))].sort();
  ...
  var finalGrid = document.getElementById('mfgGrid');
  if(!finalGrid) return;
  finalGrid.innerHTML = brands.map(function(b){
...
async function openBrandProfile(id){
  ...
  var loc = (brand.city||'') + ((brand.city&&brand.country)?', ':'') + (brand.country||'');
  var cats = (brand.categories||[]).join(' · ');

  var imgs = (brand.images || brand.gallery || []);
  var galleryTiles;
  if(imgs.length){
    galleryTiles = imgs.map(function(u){ return '<div style="aspect-ratio:4/3;border-radius:12px;overflow:hidden;background:var(--off)"><img src="'+u+'" alt="" style="width:100%;height:100%;object-fit:cover"></div>'; }).join('');
  } else {
    galleryTiles = '<div style="grid-column:1/-1;padding:38px 20px;border-radius:12px;border:1px dashed var(--border);background:var(--off);display:flex;flex-direction:column;align-items:center;gap:10px;color:var(--muted)"><svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg><div style="font-size:12px;font-weight:600">Company photos coming soon</div></div>';
  }
  ...
  page.innerHTML =
    ...
    + '<div style="position:absolute;inset:0;background:radial-gradient(ellipse at 80% -20%,rgba(201,168,76,.20) 0%,transparent 55%)"></div>'
    + '<div style="max-width:1180px;margin:0 auto;padding:20px 48px 0;position:relative">'
    + '<button onclick="backToBrands()" style="background:rgba(255,255,255,.12);border:none;color:#fff;font-size:12px;font-weight:600;padding:8px 14px;border-radius:8px;cursor:pointer;font-family:Manrope,sans-serif">\u2190 Back to Brands</button>'
    + '</div>'
    + '<div style="max-width:1180px;margin:0 auto;padding:22px 48px 38px;position:relative;display:flex;align-items:flex-end;gap:24px;flex-wrap:wrap">'
    + '<div style="width:96px;height:96px;border-radius:18px;background:#fff;display:flex;align-items:center;justify-content:center;font-size:30px;font-weight:900;color:var(--navy);flex-shrink:0;box-shadow:0 12px 30px rgba(0,0,0,.28)">' + initials + '</div>'
    + '<div style="flex:1;min-width:240px">'
    + (brand.featured ? '<span style="display:inline-block;font-size:9px;font-weight:800;color:var(--navy);background:var(--gold);padding:3px 10px;border-radius:100px;text-transform:uppercase;letter-spacing:1px;margin-bottom:10px">Featured Partner</span>' : '')
    + '<h1 style="font-family:Fraunces,serif;font-size:38px;font-weight:300;color:#fff;line-height:1.1;margin:0 0 6px">' + (brand.name||'') + '</h1>'
    + '<div style="font-size:13px;color:rgba(255,255,255,.65)">' + loc + (brand.founded?' · Est. '+brand.founded:'') + '</div>'
    + (cats ? '<div style="font-size:11px;color:var(--gold);font-weight:600;margin-top:9px;text-transform:uppercase;letter-spacing:.6px">' + cats + '</div>' : '')
    + '</div>'
    + (brand.website ? '<a href="' + brand.website + '" target="_blank" style="background:var(--gold);color:var(--navy);font-size:12px;font-weight:800;padding:11px 20px;border-radius:9px;text-decoration:none;white-space:nowrap">Visit Website \u2192</a>' : '')
    + '</div></div>'
    + '<div style="max-width:1180px;margin:0 auto;padding:36px 48px 64px">'
    + '<div class="bp-cols" style="display:grid;grid-template-columns:1fr 300px;gap:40px;align-items:start">'
    + '<div>'
    + '<div style="font-size:10px;font-weight:800;color:var(--muted);text-transform:uppercase;letter-spacing:1px;margin-bottom:12px">About</div>'
    + '<p style="font-size:14px;color:var(--text);line-height:1.85">' + (brand.description||'No description provided yet.') + '</p>'
    + '</div>'
    + '<div style="background:var(--off);border:1px solid var(--border);border-radius:14px;padding:22px;display:flex;flex-direction:column;gap:15px">'
    + statRow('Products', list.length)
    + statRow('Founded', brand.founded||'\u2014')
    + statRow('Employees', brand.employees||'\u2014')
    + statRow('Headquarters', loc||'\u2014')
    + '</div>'
    + '</div>'
    + '<div style="margin-top:44px">'
    + '<div style="font-size:10px;font-weight:800;color:var(--muted);text-transform:uppercase;letter-spacing:1px;margin-bottom:14px">Gallery</div>'
    ...
    + '<div><h2 style="font-family:Fraunces,serif;font-size:24px;font-weight:300;color:var(--navy)">Products by ' + (brand.name||'this brand') + '</h2>'
    + '<div style="font-size:11px;color:var(--muted);margin-top:3px">' + list.length + ' product' + (list.length!==1?'s':'') + '</div></div>'
    + bpToolbar
    + '</div>'
...
function bpEmpty(){ return '<div style="grid-column:1/-1;text-align:center;padding:50px 20px;border:1px dashed var(--border);border-radius:12px;color:var(--muted);font-size:13px">No products listed yet.</div>'; }
