...
// ============ MULTI-BRAND SAMPLE REQUEST ============
window._srAllBrands = window._srAllBrands || [];
window._srSelected  = window._srSelected  || [];
window._srMaxBrands = 5;

async function srLoadBrands(){
  // Reuse the RFQ brand cache if it's already loaded (same source of truth)
  ...
  try {
    ...
    window._rfqAllBrands = window._srAllBrands; // share the cache
...
function srRenderChips(){
  ...
  if(!chips) return;
  chips.innerHTML = window._srSelected.map(function(b){
    return '<span class="rfq-chip" data-id="'+b.id+'">'+ b.name +
           '<button type="button" class="rfq-chip-x" onclick="srRemoveBrand('+b.id+');event.stopPropagation()" aria-label="Remove">×</button></span>';
  }).join('');
  const help = document.getElementById('srBrandHelp');
  if(help){
    const n = window._srSelected.length;
    help.classList.remove('error');
    help.textContent = n===0 ? 'Pick 1 to 5 manufacturers to request samples from.'
                     : n>=window._srMaxBrands ? 'You\'ve reached the maximum of 5 brands.'
                     : (n+' selected · up to '+(window._srMaxBrands-n)+' more');
  }
}

function srAddBrand(b){
  if(window._srSelected.length >= window._srMaxBrands) return;
  if(window._srSelected.some(function(x){ return x.id===b.id; })) return;
  window._srSelected.push(b);
  srRenderChips();
  const inp = document.getElementById('srBrandSearch');
  if(inp){ inp.value=''; srSearchBrands(''); inp.focus(); }
}

function srRemoveBrand(id){
  window._srSelected = window._srSelected.filter(function(b){ return b.id !== id; });
  srRenderChips();
}
...
document.addEventListener('mousedown', function(e){
  const picker = document.getElementById('srBrandPicker');
  if(!picker) return;
  if(!picker.contains(e.target)) srCloseResults();
});
...
function srSearchBrands(q){
  ...
  q = (q||'').toLowerCase().trim();
  const selectedIds = new Set(window._srSelected.map(function(b){ return b.id; }));
  const matched = all.filter(function(b){
    if(!q) return true;
    return (b.name||'').toLowerCase().indexOf(q) >= 0 ||
           (b.country||'').toLowerCase().indexOf(q) >= 0 ||
  ...
  list.innerHTML = matched.map(function(b){
    ...
    const meta = [loc, cats].filter(Boolean).join(' \u2014 ');
    return '<div class="rfq-result'+(taken?' disabled':'')+'" '+
           (taken?'':'onmousedown="event.preventDefault();event.stopPropagation();srPickBrand('+b.id+')"')+'>'+
...
function srPickerKey(e){
  const inp = e.target;
  if(e.key === 'Backspace' && !inp.value && window._srSelected.length){
    srRemoveBrand(window._srSelected[window._srSelected.length-1].id);
  } else if(e.key === 'Escape'){
    srCloseResults();
  }
}
...
function srClearForm(){
  window._srSelected = [];
  srRenderChips();
  ['srFirst','srLast','srEmail','srPhone','srCompany','srProjectName','srDeliveryAddress','srNotes','srRole'].forEach(function(id){
    const el = document.getElementById(id); if(el) el.value='';
  });
...
async function submitSR(){
  ...
  const pname   = (document.getElementById('srProjectName')||{}).value || '';
  const addr    = (document.getElementById('srDeliveryAddress')||{}).value || '';
  const notes   = (document.getElementById('srNotes')||{}).value || '';
  const brands  = window._srSelected || [];
  const btn = document.querySelector('#page-samplerequest .btn-rfq-submit');

  if(!brands.length){
    srSetStatus('\u26a0 Please select at least one manufacturer.', 'error');
    return;
  }
  if(!first.trim() || !last.trim() || !email.trim() || !phone.trim() || !company.trim() || !role || !pname.trim() || !addr.trim() || !notes.trim()){
    srSetStatus('\u26a0 Please complete all fields before sending.', 'error');
    return;
  }

  if(btn){ btn.textContent='Sending\u2026'; btn.disabled=true; }
  srSetStatus('Sending your sample request to '+brands.length+' brand'+(brands.length!==1?'s':'')+'\u2026');

  // sample_requests already exists with these columns. We append "[Brand: X] | Project: Y" into notes
  // so brand/project context survives without needing a schema change.
  const rows = brands.map(function(b){
    const fullNotes = '[Brand: ' + b.name + ' | Project: ' + pname.trim() + ']\n\n' + notes.trim();
    return {
      ...
      address:    addr.trim(),
      notes:      fullNotes,
      product_name: 'Sample Request — ' + b.name
    };
  });

  try {
    const {error} = await sb.from('sample_requests').insert(rows);
    if(error) throw error;
    srSetStatus('\u2713 Sample request sent to '+brands.length+' brand'+(brands.length!==1?'s':'')+'. They\'ll ship samples directly to your address.', 'success');
    if(btn){ btn.textContent='Sent \u2713'; btn.style.background='#059669'; }
    setTimeout(function(){
      srClearForm();
...
// Reset the form whenever the user lands on this page so it never carries stale values
(function(){
  var _orig = window.showPage;
  if(typeof _orig !== 'function') return;
  window.showPage = function(p){
