...
// Filter the Discover product grid by category (Structure/Envelope/.../All).
function pickDiscoverCat(btn){
  ...
  window._discoverCat = cat;
  var grid = document.getElementById('homeProdGrid');
  if(!grid) return;
  var cards = grid.querySelectorAll('.prod-card');
  cards.forEach(function(card){
    if(cat==='all'){ card.style.display=''; return; }
    var tag = card.querySelector('.prod-cat-tag');
    var cardCat = tag ? tag.textContent.trim() : '';
    card.style.display = (cardCat===cat) ? '' : 'none';
  });
}

// When the user changes Discover tabs, re-apply the current category filter once the new cards render.
(function(){
  var orig = window.switchDiscoverTab;
  if(typeof orig === 'function'){
    window.switchDiscoverTab = function(btn){
      orig(btn);
      setTimeout(function(){
        var active = document.querySelector('.discover-cat-pill.active');
        if(active && active.getAttribute('data-cat') !== 'all'){ pickDiscoverCat(active); }
      }, 280);
    };
  }
})();
