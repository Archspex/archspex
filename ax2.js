...
function doSearch(){
  ...
  // Navigate to Products FIRST — link_shim's reset-on-nav wrapper will fire
  // here and wipe filter state (including any stale _activeSearchQ). We then
  // set _activeSearchQ AFTER, so the search survives that reset. The
  // _searchInProgress flag tells the (deferred) reset not to clear the query.
  window._searchInProgress = true;
  showPage('products');
  window._activeSearchQ = q;
  // Clear the guard after two frames — long enough for the deferred reset
  // to have finished checking it, but short enough that later user actions
  // (clearing the search, navigating away) work normally.
  setTimeout(function(){ window._searchInProgress = false; }, 200);
  // Kick a render right away — if liveProducts is already populated, we get
  // filtered results immediately. If it's still loading, the sticky-search
  // logic inside applyAndRender will pick up the query when renderAllProducts's
  // async fetch completes and it calls applyAndRender again.
