...
async function renderDashboard(){
  ...
  document.getElementById('dashRfqList').innerHTML = rfqs?.length
    ? rfqs.slice(0,3).map(r=>`<div style="padding:10px 0;border-bottom:1px solid var(--border)"><div style="font-weight:600;color:var(--text)">${r.project_name||'Untitled Project'}</div><div style="font-size:10px;color:var(--muted);margin-top:2px">${r.brand_name||r.project_type||'—'} · ${new Date(r.created_at).toLocaleDateString('en-GB')}</div></div>`).join('')
...
// Renders a single inbox card (RFQ or sample). Card type is in `kind`.
function inboxCardHtml(r, kind){
  ...
  const statusColor = status==='new' ? 'var(--gold)' : status==='replied' ? '#059669' : status==='viewed' ? '#4f8ef7' : 'var(--muted)';
  // Submitter shape differs between rfqs and sample_requests
  ...
  const submitterEmail   = kind==='rfq' ? r.submitter_email   : r.email;
  const submitterPhone   = kind==='rfq' ? r.submitter_phone   : r.phone;
  const title = kind==='rfq'
    ? (r.project_name || 'Untitled project')
    : (r.product_name || 'Sample request');
  const meta = kind==='rfq'
    ? [r.project_type, r.project_location, r.required_by].filter(Boolean).join(' \u00b7 ')
    : (r.address || '');
  const messageField = kind==='rfq' ? (r.message||'') : (r.notes||'');
  return '<div style="background:white;border:1px solid var(--border);border-radius:14px;padding:20px 22px;display:flex;flex-direction:column;gap:10px">'
    + '<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap">'
      + '<div>'
        + '<div style="font-family:Fraunces,serif;font-size:18px;color:var(--navy2);font-weight:400;line-height:1.25">' + title + '</div>'
        + '<div style="font-size:11px;color:var(--muted);margin-top:3px">' + when + ' \u00b7 From ' + (submitter || '\u2014') + (submitterCompany ? ' (' + submitterCompany + ')' : '') + '</div>'
      + '</div>'
    ...
    + (messageField ? '<div style="font-size:13px;color:var(--text);line-height:1.65;background:var(--off);padding:12px 14px;border-radius:10px;white-space:pre-wrap">' + (messageField.length>320?messageField.substring(0,320)+'\u2026':messageField) + '</div>' : '')
    + '<div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;padding-top:6px;border-top:1px solid var(--border)">'
      + '<button onclick="openInboxReply(\''+kind+'\','+r.id+')" class="btn-sm-navy">View &amp; reply</button>'
      + (submitterEmail ? '<a href="mailto:' + submitterEmail + '?subject=Re: ' + encodeURIComponent(title) + '" style="background:white;color:var(--navy);border:1.5px solid var(--border);font-size:11px;font-weight:700;padding:8px 14px;border-radius:8px;text-decoration:none">Email</a>' : '')
      + (submitterPhone ? '<a href="tel:' + submitterPhone + '" style="background:white;color:var(--navy);border:1.5px solid var(--border);font-size:11px;font-weight:700;padding:8px 14px;border-radius:8px;text-decoration:none">Call</a>' : '')
    + '</div>'
  + '</div>';
}
