...
function toggleSortDD(btn){
  var dd = btn.closest('.sort-dd');
  if(!dd) return;
  var wasOpen = dd.classList.contains('open');
  document.querySelectorAll('.sort-dd').forEach(function(d){ d.classList.remove('open'); });
  if(!wasOpen) dd.classList.add('open');
}
function pickSortOpt(opt){
  var dd = opt.closest('.sort-dd');
  if(!dd) return;
...
async function submitBrandApplication(){
  ...
  const {error} = await sb.from('brand_applications').insert({
    ...
    phone, country, message: `[Membership tier requested: ${tier}]` + (message ? '\n\n' + message : '')
  });
  ...
  ['ba-company','ba-name','ba-email','ba-phone','ba-country','ba-message','ba-tier'].forEach(id=>{
