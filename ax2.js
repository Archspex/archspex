...
// ── PRODUCT PAGE (full standalone view — replaces the popup modal) ─────────────
async function openProduct(id){
  ...
  if(!p){
    ...
    var _pt = document.getElementById('ppTitle'); if(_pt) _pt.textContent = 'Loading…';
    try {
    ...
    if(!p){ var _e=document.getElementById('ppTitle'); if(_e) _e.textContent='Could not load product.'; return; }
  }
...
function renderProductPage(p){
  ...
  set('ppBcType', subcat);
  set('ppBcName', name);
  set('ppCat', (cat||'').toUpperCase());
  set('ppSubcat', (subcat||'').toUpperCase());
  set('ppTitle', name);
  set('ppDesc', desc || 'Specification-ready product with technical documentation and BIM/CAD support.');
  set('ppLongDesc', desc || 'Product description is not available yet.');
  set('ppAboutDesc', desc || 'Detailed product description with features, benefits and use cases.');

  var mainImg = document.getElementById('ppMainImg');
  if(mainImg){
    mainImg.onerror = function(){ this.src = 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=800'; };
    mainImg.src = img || 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=800';
  }
  var thumbs = document.getElementById('ppThumbs');
  if(thumbs){
    thumbs.innerHTML = '';
    var urls = [img, img, img, img].filter(function(u){ return !!u; });
    urls.forEach(function(u, i){
      var t = document.createElement('div');
      t.className = 'pp-thumb' + (i===0?' active':'');
      t.innerHTML = '<img src="'+u+'" alt="">';
      t.onclick = function(){
        document.querySelectorAll('.pp-thumb').forEach(function(x){ x.classList.remove('active'); });
  ...
  var initials = (brand||'?').split(/\s+/).map(function(w){ return (w[0]||'').toUpperCase(); }).join('').slice(0,3);
  set('ppBrandInitials', initials);
  set('ppBrand', (brand||'').toUpperCase());
  set('ppBrandCountry', country);
  set('ppBrandEst', specs.established || '—');

  set('ppSpProductType', specs.product_type || specs.type || subcat || cat);
  set('ppSpMaterial',    specs.material || '—');
  set('ppSpApplications',specs.applications || specs.application || 'Interior Wall Finishes');
  set('ppSpInstallation',specs.installation || 'Wall Mounted');
  set('ppSpFire',        specs.fire_rating || specs.fire || '—');
  set('ppSpWarranty',    specs.warranty || '—');

  set('ppTechFire',      specs.fire_rating || 'Class B-s1,d0');
  set('ppTechAcoustic',  specs.acoustic || specs.acoustic_performance || 'NRC up to 0.85');
  set('ppTechMaterial',  specs.material || '100% Recycled PET');
  set('ppTechThickness', specs.thickness || '9mm / 12mm / 24mm');
  set('ppTechSizes',     specs.sizes || specs.size || '2400 × 1200mm');
  set('ppTechWarranty',  specs.warranty || '10 Years');
  set('ppTechOrigin',    country || specs.origin || '—');
  set('ppTechInstall',   specs.installation || 'Wall Mounted');
  set('ppTechEdges',     specs.edge_profiles || specs.edges || 'Square / Bevel / Shadow Gap');

  set('ppMfgInitials', initials);
  set('ppMfgName', (brand||'').toUpperCase());
  set('ppMfgCountry', country);
  set('ppMfgDesc', specs.brand_desc || 'Sustainable materials manufacturer supplying specification-ready products for architects, designers and contractors across the GCC.');
  set('ppMfgEst', specs.established || '—');
  set('ppMfgHq', specs.headquarters || country || '—');
  set('ppMfgWeb', specs.website || '—');
  set('ppMfgLink', brand || 'this brand');
}

function ppSwitchTab(evt, tabName){
  document.querySelectorAll('#ppTabs .pp-tab').forEach(function(t){ t.classList.remove('active'); });
  document.querySelectorAll('.pp-tabpane').forEach(function(p){ p.classList.remove('active'); });
  if(evt && evt.currentTarget) evt.currentTarget.classList.add('active');
  var pane = document.querySelector('.pp-tabpane[data-tab="'+tabName+'"]');
  if(pane) pane.classList.add('active');
}

function ppScrollCarousel(id, dir){
  var el = document.getElementById(id); if(!el) return;
  var step = Math.round(el.clientWidth * 0.7) * dir;
