...
function clearAllFilters(){
  ...
  window._activeSearchQ = ''; // also clear any active search when filters are wiped
...
function applyAndRender(){
  ...
  if(searchQ){
    ...
    filtered = prods.filter(function(p){
      var hay = ((p.name||'') + ' ' + (p.brand||'') + ' ' + (p.cat||p.category||'') + ' ' +
                 (p.subtype||p.meta||'') + ' ' + (p.country||'') + ' ' + (p.description||'')).toLowerCase();
...
// ── SEARCH ────────────────────────────────────────────────────────────────────
function doSearch(){
  // Prefer whichever input the user typed into; fall back to the other.
  const heroEl = document.getElementById('heroSearchInput');
  const navEl  = document.getElementById('navSearchInput');
  const rawQ = ((heroEl && heroEl.value) || (navEl && navEl.value) || '').trim();
  ...
  // Remember the active search so async renders (renderAllProducts finishing
  // its DB fetch, filter-reset re-renders, etc.) can defer to us.
  window._activeSearchQ = q;
  showPage('products');
  // Wait until liveProducts is populated before rendering results — otherwise
  // renderAllProducts's async fetch completes AFTER our filter and overwrites
  // the results with the full product list.
  let attempts = 0;
  (function applyFilter(){
    const ready = Array.isArray(liveProducts) && liveProducts.length > 0;
    if(!ready && attempts++ < 60){
      return setTimeout(applyFilter, 100);
    }
    // If the user has since cleared or changed the search, abort.
    if(window._activeSearchQ !== q) return;
    const all=[...(liveProducts||[])];
    // Split the query into terms; every term must match SOMEWHERE in the
    // product's searchable fields (name, brand, category, subtype/meta,
    // country, description). Case-insensitive.
    const terms = q.split(/\s+/).filter(Boolean);
    const f = all.filter(p => {
      const hay = ((p.name||'') + ' ' + (p.brand||'') + ' ' + (p.cat||p.category||'') + ' '
                 + (p.subtype||p.meta||'') + ' ' + (p.country||'') + ' ' + (p.description||'')).toLowerCase();
