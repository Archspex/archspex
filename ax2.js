...
async function openInboxReply(kind, id){
  ...
  // The original request message — quoted block
  const messageBlock = message ? '<div>'
    + '<div style="font-size:9px;font-weight:800;color:var(--muted);text-transform:uppercase;letter-spacing:.8px;margin-bottom:8px">Their request</div>'
    + '<div style="font-size:13px;color:var(--text);line-height:1.7;background:#fafbfd;border:1px solid var(--border);padding:14px 16px;border-radius:10px;white-space:pre-wrap">' + message + '</div>'
  + '</div>' : '';

  document.getElementById('irBody').innerHTML = senderCard + detailsGrid + messageBlock;
