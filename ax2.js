...
function rfqSearchBrands(q){
  ...
  list.innerHTML = matched.map(function(b){
    ...
    return '<div class="rfq-result'+(taken?' disabled':'')+'" '+
           (taken?'':'onclick="rfqPickBrand('+b.id+')"')+'>'+
...
function rfqClearForm(){
  window._rfqSelected = [];
  rfqRenderChips();
  ['rfqFirst','rfqLast','rfqEmail','rfqPhone','rfqCompany','rfqProjectName','rfqProjectLocation','rfqRequiredBy','rfqMessage'].forEach(function(id){
    const el = document.getElementById(id); if(el) el.value='';
  });
  ['rfqRole','rfqProjectType'].forEach(function(id){
...
// Preload brand list when navigating to the RFQ page so the search is instant
(function(){
  var _orig = window.showPage;
  if(typeof _orig !== 'function') return;
  window.showPage = function(p){
    var r = _orig.apply(this, arguments);
    if(p === 'rfq'){ try { rfqLoadBrands(); rfqRenderChips(); } catch(e){} }
    return r;
  };
})();
