...
async function openBrandProfile(id){
  ...
  if(!page){
    ...
    (ref ? ref.parentNode : document.body).appendChild(page);
  }
  ...
  var prodGrid = list.length
    ? '<div class="prod-grid">' + list.map(prodCard).join('') + '</div>'
    : '<div style="text-align:center;padding:50px 20px;border:1px dashed var(--border);border-radius:12px;color:var(--muted);font-size:13px">No products listed yet.</div>';
  ...
  page.innerHTML =
    ...
    + '<h2 style="font-family:Fraunces,serif;font-size:24px;font-weight:300;color:var(--navy)">Products by ' + (brand.name||'this brand') + '</h2>'
    + '<span style="font-size:12px;color:var(--muted)">' + list.length + ' product' + (list.length!==1?'s':'') + '</span>'
    + '</div>'
    + prodGrid
    + '</div>'
    + '</div>';
