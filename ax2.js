...
// IMPORTANT: don't clobber deep-link hashes like #product/123 or #brand-xyz —
// those need to survive so prod_shim / brand_shim can pick them up on load
// (Open-in-new-tab / refresh flows). Only synthesise #home when there's
// truly no hash at all.
(function(){
  var _h = location.hash || '';
  if (!_h) {
    history.replaceState({page:'home'}, '', '#home');
  } else if (_h.indexOf('#product/') === 0) {
    history.replaceState({page:'product', pid: _h.replace('#product/','')}, '', _h);
  } else if (_h.indexOf('#brand-') === 0 || _h.indexOf('#brand/') === 0) {
    history.replaceState({page:'brandprofile'}, '', _h);
  } else {
    // Plain page hash like #products / #manufacturers — seed a state object so
    // future popstate events resolve back to this page instead of falling back to home.
    history.replaceState({page:_h.substring(1)}, '', _h);
  }
})();
...
function prodCard(p){
  ...
  var clickGuard = 'if(event.metaKey||event.ctrlKey||event.shiftKey||event.button===1)return true;event.preventDefault();';
  return '<a class="prod-card" href="' + href + '" onclick="' + clickGuard + 'openProduct(' + pidSafe + ')">'
    ...
    + '<button class="prod-wish" data-tt="Save" onclick="event.preventDefault();event.stopPropagation();handleWish(' + pidSafe + ',this)"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg></button>'
    + '</div>'
    + '<div class="prod-body">'
    ...
    + '<div class="prod-foot">'
    + '<button class="btn-sm-navy" onclick="event.preventDefault();event.stopPropagation();openReq(' + pidSafe + ')">Request Sample</button>'
    + '<button class="btn-sm-outline" onclick="event.preventDefault();event.stopPropagation();openProduct(' + pidSafe + ')">Spec ↓</button>'
    + '</div></div></a>';
}
...
async function renderDashboard(){
  ...
  if(saved?.length){
    document.getElementById('dashSavedGrid').innerHTML = saved.map(p=>`
      <a class="prod-card" href="#product/${encodeURIComponent(String(p.product_id))}" onclick="if(event.metaKey||event.ctrlKey||event.shiftKey||event.button===1)return true;event.preventDefault();openProduct(${p.product_id})">
...
async function renderManufacturers(){
  ...
  var btitleEl = document.getElementById('brandsPageTitle');
