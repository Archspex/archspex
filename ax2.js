...
(function(){try{history.scrollRestoration='manual';}catch(e){}var hp=(location.hash||'').replace('#','')||'home';history.replaceState({page:hp},'','#'+hp);if(hp!=='home'){setTimeout(function(){showPage(hp,false);window.scrollTo(0,0);},0);}else{window.scrollTo(0,0);}})();
...
function prodCard(p){
  ...
  return '<div class="prod-card" onclick="openProduct(' + pidSafe + ')">'
    ...
    + '<button class="prod-wish" onclick="event.stopPropagation();handleWish(' + pidSafe + ',this)" data-tt="Save" style="color:var(--navy)"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg></button>'
    + '<button class="prod-compare-btn" onclick="event.stopPropagation();toggleCompare(' + pidSafe + ',this)" data-tt="Compare" style="color:var(--navy)"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><polyline points="16 3 21 3 21 8"/><line x1="4" y1="20" x2="21" y2="3"/><polyline points="21 16 21 21 16 21"/><line x1="15" y1="15" x2="21" y2="21"/><line x1="4" y1="4" x2="9" y2="9"/></svg></button>'
    + '</div>'
    + '<div class="prod-body">'
    + '<div class="prod-name">' + (p.name||'') + '</div>'
    + '<div class="prod-type">' + (p.meta||'') + '</div>'
    + '<div class="prod-brand-row"><span class="prod-brand">' + (p.brand||'') + '</span>' + (countryTC ? ' · <span class="prod-country">' + countryTC + '</span>' : '') + '</div>'
    + iconsHtml
    + '<div class="prod-foot">'
    + '<button class="btn-sm-navy prod-view-btn" onclick="event.stopPropagation();openProduct(' + pidSafe + ')">View Product</button>'
    + '</div></div></div>';
}
...
// Inject compare button styles (keeps index.html untouched)
(function(){
  var st = document.createElement('style');
  st.textContent = '.prod-compare-btn{position:absolute;top:42px;right:8px;width:28px;height:28px;border-radius:50%;background:white;border:none;display:flex;align-items:center;justify-content:center;opacity:0;transition:all .2s;box-shadow:0 2px 8px rgba(0,0,0,.12);cursor:pointer}'
    + '.prod-card:hover .prod-compare-btn{opacity:1}'
    + '.prod-compare-btn.active{background:var(--gold);opacity:1}'
    + '.prod-compare-btn.active svg{stroke:var(--navy)}'
    + '@media(max-width:768px){.prod-compare-btn{opacity:1}}';
  document.head.appendChild(st);
})();

function getProductForCompare(id){
  var sid = String(id);
  if(window._prodCache && window._prodCache[sid]) return window._prodCache[sid];
  var all = [].concat(typeof products !== 'undefined' ? products : [], typeof liveProducts !== 'undefined' ? liveProducts : []);
  return all.find(function(x){ return String(x.id) === sid || String(x.db_id) === sid; });
}

function toggleCompare(id, btn){
  var sid = String(id);
  var idx = compareList.indexOf(sid);
  if(idx >= 0){
    compareList.splice(idx, 1);
    if(btn) btn.classList.remove('active');
  } else {
    if(compareList.length >= 2){
      showToast('You can compare 2 products — remove one first');
      return;
    }
...
function updateCompareBar(){
  ...
  if(!bar){
    ...
    bar.id = 'compareBar';
    bar.style.cssText = 'position:fixed;bottom:0;left:0;right:0;z-index:800;background:var(--navy);padding:12px 24px;display:none;align-items:center;justify-content:space-between;gap:16px;box-shadow:0 -4px 24px rgba(0,0,0,.2);flex-wrap:wrap';
    document.body.appendChild(bar);
  }
  if(compareList.length === 0){ bar.style.display = 'none'; return; }
  var items = compareList.map(function(id){
    var p = getProductForCompare(id);
    var img = p ? (p.img || p.image_url || '') : '';
    var name = p ? (p.name || '') : 'Product';
    return '<div style="display:flex;align-items:center;gap:8px;background:rgba(255,255,255,.08);border-radius:8px;padding:6px 12px 6px 6px">'
      + '<img src="' + img + '" style="width:36px;height:36px;object-fit:cover;border-radius:6px" onerror="this.src=\'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=100\'">'
      + '<span style="font-size:11px;color:white;font-weight:600;max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + name + '</span>'
      + '<button onclick="removeFromCompare(\'' + id + '\')" style="background:none;border:none;color:rgba(255,255,255,.5);font-size:14px;cursor:pointer;padding:0 4px">&times;</button>'
      + '</div>';
  }).join('');
  bar.innerHTML = '<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">'
    + '<span style="font-size:11px;font-weight:800;color:var(--gold);text-transform:uppercase;letter-spacing:1px">Compare (' + compareList.length + '/2)</span>'
    + items
    + (compareList.length < 2 ? '<span style="font-size:11px;color:rgba(255,255,255,.4)">Select 1 more product</span>' : '')
    + '</div>'
    + '<div style="display:flex;gap:8px;flex-shrink:0">'
    + (compareList.length === 2 ? '<button onclick="openCompareView()" style="background:var(--gold);color:var(--navy);border:none;border-radius:6px;padding:9px 20px;font-size:12px;font-weight:800;cursor:pointer;font-family:Manrope,sans-serif">Compare Now &rarr;</button>' : '')
    + '<button onclick="clearCompare()" style="background:transparent;color:rgba(255,255,255,.6);border:1px solid rgba(255,255,255,.2);border-radius:6px;padding:9px 16px;font-size:11px;font-weight:600;cursor:pointer;font-family:Manrope,sans-serif">Clear</button>'
    + '</div>';
  bar.style.display = 'flex';
}

function removeFromCompare(id){
  var idx = compareList.indexOf(String(id));
  if(idx >= 0) compareList.splice(idx, 1);
  document.querySelectorAll('.prod-compare-btn.active').forEach(function(b){ b.classList.remove('active'); });
  updateCompareBar();
}
...
function openCompareView(){
  if(compareList.length !== 2){ showToast('Select 2 products to compare'); return; }
  var p1 = getProductForCompare(compareList[0]);
  var p2 = getProductForCompare(compareList[1]);
  if(!p1 || !p2){ showToast('Could not load products'); return; }
  ...
  var s1 = parseSpecs(p1), s2 = parseSpecs(p2);
  var allKeys = Object.keys(s1);
  Object.keys(s2).forEach(function(k){ if(allKeys.indexOf(k) < 0) allKeys.push(k); });

  function row(label, v1, v2, highlight){
    var same = String(v1||'—') === String(v2||'—');
    return '<div style="display:grid;grid-template-columns:140px 1fr 1fr;border-top:1px solid var(--border)">'
      + '<div style="padding:11px 14px;font-size:10px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;background:var(--off)">' + label + '</div>'
      + '<div style="padding:11px 14px;font-size:12px;color:var(--text);font-weight:' + (highlight&&!same?'700':'400') + '">' + (v1||'—') + '</div>'
      + '<div style="padding:11px 14px;font-size:12px;color:var(--text);border-left:1px solid var(--border);font-weight:' + (highlight&&!same?'700':'400') + '">' + (v2||'—') + '</div>'
      + '</div>';
  }
  var specRows = allKeys.map(function(k){ return row(k, s1[k], s2[k], true); }).join('');
  var id1 = p1.id||p1.db_id, id2 = p2.id||p2.db_id;

  var overlay = document.createElement('div');
  overlay.id = 'compareOverlay';
  overlay.style.cssText = 'position:fixed;inset:0;z-index:7000;background:rgba(0,10,30,.7);backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;padding:20px';
  overlay.onclick = function(e){ if(e.target === overlay) overlay.remove(); };

  overlay.innerHTML = '<div style="background:white;border-radius:10px;max-width:860px;width:100%;max-height:90vh;overflow-y:auto;position:relative">'
    + '<div style="background:var(--navy);padding:20px 28px;border-radius:10px 10px 0 0;display:flex;justify-content:space-between;align-items:center;position:sticky;top:0;z-index:10">'
    + '<div><div style="font-size:9px;font-weight:800;color:var(--gold);text-transform:uppercase;letter-spacing:1.5px;margin-bottom:4px">Product Comparison</div>'
    + '<div style="font-family:Fraunces,serif;font-size:20px;font-weight:300;color:white">Compare Side by Side</div></div>'
    + '<button onclick="document.getElementById(\'compareOverlay\').remove()" style="background:rgba(255,255,255,.15);border:none;color:white;width:32px;height:32px;border-radius:50%;font-size:16px;cursor:pointer">&times;</button>'
    + '</div>'
    + '<div style="display:grid;grid-template-columns:140px 1fr 1fr">'
    + '<div style="background:var(--off)"></div>'
    + '<div style="padding:20px 14px;text-align:center">'
    + '<img src="' + (p1.img||p1.image_url||'') + '" style="width:100%;height:140px;object-fit:cover;border-radius:8px;margin-bottom:12px" onerror="this.src=\'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=400\'">'
    + '<div style="font-size:14px;font-weight:800;color:var(--text);margin-bottom:4px">' + (p1.name||'') + '</div>'
    + '<div style="font-size:11px;color:var(--navy);font-weight:700">' + (p1.brand||'') + '</div>'
    + '</div>'
    + '<div style="padding:20px 14px;text-align:center;border-left:1px solid var(--border)">'
    + '<img src="' + (p2.img||p2.image_url||'') + '" style="width:100%;height:140px;object-fit:cover;border-radius:8px;margin-bottom:12px" onerror="this.src=\'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=400\'">'
    + '<div style="font-size:14px;font-weight:800;color:var(--text);margin-bottom:4px">' + (p2.name||'') + '</div>'
    + '<div style="font-size:11px;color:var(--navy);font-weight:700">' + (p2.brand||'') + '</div>'
    + '</div>'
    + '</div>'
    + row('Category', p1.cat||p1.category, p2.cat||p2.category)
    + row('Brand', p1.brand, p2.brand)
    + row('Country', p1.country, p2.country)
    + row('Type', p1.meta, p2.meta)
    + (specRows ? '<div style="padding:12px 14px;background:var(--navy);font-size:10px;font-weight:800;color:var(--gold);text-transform:uppercase;letter-spacing:1px">Technical Specifications</div>' + specRows : '')
    + row('Description', (p1.description||p1.desc||'').substring(0,140), (p2.description||p2.desc||'').substring(0,140))
    + '<div style="display:grid;grid-template-columns:140px 1fr 1fr;border-top:1px solid var(--border)">'
    + '<div style="background:var(--off)"></div>'
    + '<div style="padding:16px 14px;display:flex;gap:8px;flex-direction:column">'
    + '<button onclick="document.getElementById(\'compareOverlay\').remove();openProduct(\'' + id1 + '\')" style="background:var(--navy);color:white;border:none;border-radius:6px;padding:10px;font-size:11px;font-weight:700;cursor:pointer;font-family:Manrope,sans-serif">View Product</button>'
    + '<button onclick="document.getElementById(\'compareOverlay\').remove();openReq(\'' + id1 + '\')" style="background:white;color:var(--navy);border:1.5px solid var(--border);border-radius:6px;padding:10px;font-size:11px;font-weight:700;cursor:pointer;font-family:Manrope,sans-serif">Request Sample</button>'
    + '</div>'
    + '<div style="padding:16px 14px;display:flex;gap:8px;flex-direction:column;border-left:1px solid var(--border)">'
    + '<button onclick="document.getElementById(\'compareOverlay\').remove();openProduct(\'' + id2 + '\')" style="background:var(--navy);color:white;border:none;border-radius:6px;padding:10px;font-size:11px;font-weight:700;cursor:pointer;font-family:Manrope,sans-serif">View Product</button>'
    + '<button onclick="document.getElementById(\'compareOverlay\').remove();openReq(\'' + id2 + '\')" style="background:white;color:var(--navy);border:1.5px solid var(--border);border-radius:6px;padding:10px;font-size:11px;font-weight:700;cursor:pointer;font-family:Manrope,sans-serif">Request Sample</button>'
    + '</div>'
    + '</div>'
    + '</div>';
...
// ── BRAND MARQUEE (infinite scroll, clickable) ────────────────────────────────
(function(){
  var st = document.createElement('style');
  st.textContent = '.brand-marquee-wrap{background:var(--off,#f8f9fc);padding:26px 0 22px;border-bottom:1px solid var(--border,#e5e9f0);overflow:hidden;position:relative}'
    + '.brand-marquee-wrap::before,.brand-marquee-wrap::after{content:"";position:absolute;top:0;bottom:0;width:90px;z-index:2;pointer-events:none}'
    + '.brand-marquee-wrap::before{left:0;background:linear-gradient(90deg,var(--off,#f8f9fc),transparent)}'
    + '.brand-marquee-wrap::after{right:0;background:linear-gradient(-90deg,var(--off,#f8f9fc),transparent)}'
    + '.brand-marquee-label{text-align:center;font-size:9px;font-weight:800;color:var(--muted,#6b7685);letter-spacing:2.5px;text-transform:uppercase;margin-bottom:16px}'
    + '.brand-marquee{display:flex;gap:52px;animation:bmarquee 28s linear infinite;width:max-content}'
    + '.brand-marquee:hover{animation-play-state:paused}'
    + '.brand-m-item{font-size:15px;font-weight:800;color:#b5bdc9;letter-spacing:1.2px;white-space:nowrap;cursor:pointer;transition:color .25s;text-transform:uppercase;background:none;border:none;font-family:Manrope,sans-serif;padding:0}'
    + '.brand-m-item:hover{color:var(--navy,#003366)}'
    + '@keyframes bmarquee{to{transform:translateX(-50%)}}';
  document.head.appendChild(st);

  async function initBrandMarquee(){
    var home = document.getElementById('page-home');
    if(!home || document.getElementById('brandMarquee')) return;
    var anchor = home.querySelector('.slider-section');
    if(!anchor) return;

    var names = [];
    try {
      var res = await sb.from('manufacturers').select('name').eq('status','active').order('featured',{ascending:false}).limit(20);
      names = (res.data||[]).map(function(m){ return m.name; });
    } catch(e){}
    if(!names.length) return;

    var itemsHtml = names.map(function(n){
      var safe = n.replace(/'/g, "\\'");
      return '<button class="brand-m-item" onclick="openBrandByName(\'' + safe + '\')">' + n + '</button>';
    }).join('');

    var wrap = document.createElement('div');
    wrap.id = 'brandMarquee';
    wrap.className = 'brand-marquee-wrap';
    wrap.innerHTML = '<div class="brand-marquee-label">Trusted International Brands on ArchSpex</div>'
      + '<div class="brand-marquee">' + itemsHtml + itemsHtml + '</div>';
    anchor.parentNode.insertBefore(wrap, anchor.nextSibling);
  }

  if(document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', function(){ setTimeout(initBrandMarquee, 800); });
  } else {
    setTimeout(initBrandMarquee, 800);
  }
})();
...
function doSearch(){
  ...
  setTimeout(()=>{
    ...
    document.getElementById('allProdGrid').innerHTML=f.length?f.map(prodCard).join(''):`<div style="grid-column:1/-1;text-align:center;padding:80px"><div style="margin-bottom:12px;display:flex;justify-content:center"><svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="var(--muted)" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg></div><div style="font-size:16px;font-weight:700;color:var(--text);margin-bottom:6px">No results for "${q}"</div><div style="font-size:12px;color:var(--muted)">Try a different search term or browse all products</div></div>`;
...
async function refreshAllSavedBody(){
  ...
  document.getElementById('vcBody').innerHTML = '<div class="prod-grid">' + saved.map(function(p){
    ...
    return '<div class="prod-card" onclick="openProduct(' + pidJs + ')">'
      ...
      + '<div class="prod-body">'
        + '<div class="prod-brand-row">' + (p.brand||'\u2014') + '</div>'
        + '<div class="prod-name">' + (p.product_name||'\u2014') + '</div>'
        + '<div class="prod-foot" style="display:flex;gap:6px"><button class="btn-sm-navy" onclick="event.stopPropagation();allSavedRemove(' + p.id + ')">Remove</button></div>'
      + '</div>'
    + '</div>';
  }).join('') + '</div>';
}
...
(function(){
  ...
  window.setLoggedOut = function(){
    ...
    try {
      var p = (location.hash || '').replace('#','') || (window.currentPage) || '';
...
// ── INDIVIDUAL PRODUCT URLS (deep links + share) ──────────────────────────────
(function(){
  var _origOpen = window.openProduct;
  var _origClose = window.closeModal;

  // Wrap openProduct: update URL + inject Copy Link button
  window.openProduct = function(id){
    var r = _origOpen(id);
    try{
      if((location.hash||'').indexOf('#product/') !== 0 || location.hash !== '#product/'+id){
        history.pushState({page:'products', product:String(id)}, '', '#product/' + id);
      }
    }catch(e){}
    setTimeout(injectShareBtn, 300);
    return r;
  };

  // Wrap closeModal: restore URL
  window.closeModal = function(){
    _origClose();
    if((location.hash||'').indexOf('#product/') === 0){
      try{ history.pushState({page:'products'}, '', '#products'); }catch(e){}
    }
  };

  // Back/forward button support for product URLs
  ...
  function injectShareBtn(){
    var modal = document.getElementById('prodModal');
    if(!modal || document.getElementById('prodShareBtn')) {
      var b = document.getElementById('prodShareBtn');
      if(b) b.setAttribute('data-url', location.href);
      return;
    }
    ...
    btn.id = 'prodShareBtn';
    btn.setAttribute('data-tt','Copy link');
    btn.style.cssText = 'position:absolute;top:16px;right:60px;z-index:20;width:36px;height:36px;border-radius:2px;background:white;border:1px solid #e5e7eb;display:flex;align-items:center;justify-content:center;color:#6b7280;cursor:pointer;transition:all .15s';
    btn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>';
    btn.onmouseover = function(){ btn.style.background='#f3f4f6'; btn.style.color='#111'; };
    btn.onmouseout = function(){ btn.style.background='white'; btn.style.color='#6b7280'; };
    btn.onclick = function(e){
      e.stopPropagation();
      var url = location.href;
      if(navigator.clipboard && navigator.clipboard.writeText){
        navigator.clipboard.writeText(url).then(function(){ showToast('Product link copied!'); });
      } else {
        var ta = document.createElement('textarea');
        ta.value = url; document.body.appendChild(ta); ta.select();
        try{ document.execCommand('copy'); showToast('Product link copied!'); }catch(err){}
        ta.remove();
      }
    };
  ...
  // Deep link on initial page load: archspex.com/#product/123
  function initDeepLink(){
    var h = location.hash || '';
    if(h.indexOf('#product/') !== 0) return;
    var pid = h.replace('#product/','');
    try{ showPage('products', false); }catch(e){}
    // Wait for products to load into cache, retry a few times
    var tries = 0;
    (function attempt(){
      tries++;
      var cached = window._prodCache && window._prodCache[String(pid)];
      if(cached || tries > 6){
        _origOpen(pid);
        setTimeout(injectShareBtn, 300);
        try{ history.replaceState({page:'products', product:String(pid)}, '', '#product/'+pid); }catch(e){}
      } else {
  ...
  if(document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', function(){ setTimeout(initDeepLink, 600); });
  } else {
    setTimeout(initDeepLink, 600);
  }
})();
