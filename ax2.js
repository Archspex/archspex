...
async function submitReq(){
  ...
  const _addrParts = [allInputs[5]?.value || '', allInputs[6]?.value || '', allInputs[7]?.value || '', allInputs[8]?.value || '', allInputs[9]?.value || ''];
...
async function renderDashboard(){
  ...
  document.getElementById('dashRfqList').innerHTML = rfqs?.length
    ? (window._allRfqs=rfqs,window.__updateViewAll&&window.__updateViewAll(),rfqs.slice(0,10).map(r=>window.renderSubmissionCard(r,'rfq')).join(''))
  ...
  document.getElementById('dashSampleList').innerHTML = samples?.length
    ? (window._allSamples=samples,window.__updateViewAll&&window.__updateViewAll(),samples.slice(0,10).map(r=>window.renderSubmissionCard(r,'sample')).join(''))
...
function srClearForm(){
  ...
  if(wrap) wrap.style.display = 'none';
  srRenderProductChip();
  ['srFirst','srLast','srEmail','srPhone','srCompany','srAddrBuilding','srAddrStreet','srAddrArea','srAddrCity','srAddrCountry','srNotes','srRole'].forEach(function(id){
    const el = document.getElementById(id); if(el) el.value='';
  });
...
async function submitSR(){
  ...
  const addr    = [_addrBld, _addrSt, _addrAr, _addrCi, _addrCo].map(function(x){return (x||'').trim();}).filter(Boolean).join(', ');
  const _addrValid = _addrSt.trim() && _addrAr.trim() && _addrCi.trim() && _addrCo.trim();
...
window.__updateViewAll = function(){
  ...
  if (rfqBtn) rfqBtn.style.display = (window._allRfqs && window._allRfqs.length > 10) ? 'block' : 'none';
  var sBtn = document.getElementById('dashSampleViewAll');
  if (sBtn) sBtn.style.display = (window._allSamples && window._allSamples.length > 10) ? 'block' : 'none';
};
  ...
  window.openReq = async function(){
    ...
    if(currentUser){
      ...
      setTimeout(function(){
        ...
        // Country into the 5th address field (inputs[9]) if present
        if(inputs[9] && !inputs[9].value) inputs[9].value = p.country || '';
