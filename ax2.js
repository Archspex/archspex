...
async function renderDashboard(){
  ...
  document.getElementById('dashRfqList').innerHTML = rfqs?.length
    ? rfqs.slice(0,3).map(r=>{var st=(r.last_actor==='brand')?'new':(r.last_actor==='submitter'?'replied':'new');if((r.status||'').toLowerCase()==='closed')st='closed';var stColor=st==='new'?'var(--gold)':st==='replied'?'#059669':'#9ca3af';var stLabel=st.charAt(0).toUpperCase()+st.slice(1);var msgs=Array.isArray(r.messages)?r.messages:[];var lastBrand=null;for(var i=msgs.length-1;i>=0;i--){if(msgs[i].from==='brand'){lastBrand=msgs[i];break;}}if(!lastBrand&&r.reply)lastBrand={text:r.reply};var reply=lastBrand?(lastBrand.text||'').trim():'';return `<div onclick="openInboxReply('rfq',${r.id})" style="padding:10px 0;border-bottom:1px solid var(--border);cursor:pointer;transition:background .12s" onmouseover="this.style.background='var(--off)'" onmouseout="this.style.background=''"><div style="display:flex;justify-content:space-between;align-items:center;gap:8px"><div style="font-weight:600;color:var(--text);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1">${r.project_name||'Untitled Project'}</div><span style="font-size:8px;font-weight:800;color:${stColor};border:1px solid ${stColor};padding:2px 6px;border-radius:100px;text-transform:uppercase;letter-spacing:.5px">${stLabel}</span></div><div style="font-size:10px;color:var(--muted);margin-top:2px">${r.brand_name||r.project_type||'—'} · ${new Date(r.created_at).toLocaleDateString('en-GB')}</div>${reply?`<div style="font-size:11px;color:var(--text);margin-top:6px;padding:8px 10px;background:#f0fdf4;border-left:3px solid #059669;border-radius:6px;line-height:1.5"><div style="font-size:9px;font-weight:800;color:#059669;text-transform:uppercase;letter-spacing:.5px;margin-bottom:2px">Reply from ${r.brand_name||'brand'}</div>${reply.length>140?reply.substring(0,140)+'…':reply}</div>`:''}</div>`;}).join('')
  ...
  document.getElementById('dashSampleList').innerHTML = samples?.length
    ? samples.slice(0,3).map(r=>{var st=(r.last_actor==='brand')?'new':(r.last_actor==='submitter'?'replied':'new');if((r.status||'').toLowerCase()==='closed')st='closed';var stColor=st==='new'?'var(--gold)':st==='replied'?'#059669':'#9ca3af';var stLabel=st.charAt(0).toUpperCase()+st.slice(1);var msgs=Array.isArray(r.messages)?r.messages:[];var lastBrand=null;for(var i=msgs.length-1;i>=0;i--){if(msgs[i].from==='brand'){lastBrand=msgs[i];break;}}if(!lastBrand&&r.reply)lastBrand={text:r.reply};var reply=lastBrand?(lastBrand.text||'').trim():'';return `<div onclick="openInboxReply('sample',${r.id})" style="padding:10px 0;border-bottom:1px solid var(--border);cursor:pointer;transition:background .12s" onmouseover="this.style.background='var(--off)'" onmouseout="this.style.background=''"><div style="display:flex;justify-content:space-between;align-items:center;gap:8px"><div style="font-weight:600;color:var(--text);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1">${r.product_name||'Product Sample'}</div><span style="font-size:8px;font-weight:800;color:${stColor};border:1px solid ${stColor};padding:2px 6px;border-radius:100px;text-transform:uppercase;letter-spacing:.5px">${stLabel}</span></div><div style="font-size:10px;color:var(--muted);margin-top:2px">${r.company||'—'} · ${new Date(r.created_at).toLocaleDateString('en-GB')}</div>${reply?`<div style="font-size:11px;color:var(--text);margin-top:6px;padding:8px 10px;background:#f0fdf4;border-left:3px solid #059669;border-radius:6px;line-height:1.5"><div style="font-size:9px;font-weight:800;color:#059669;text-transform:uppercase;letter-spacing:.5px;margin-bottom:2px">Reply</div>${reply.length>140?reply.substring(0,140)+'…':reply}</div>`:''}</div>`;}).join('')
...
// Compute a viewer-aware status — 'NEW' if the other side acted last, 'REPLIED' if I did.
...
async function openInboxReply(kind, id){
  ...
  document.getElementById('irBody').innerHTML = senderCard + detailsGrid + threadBlock;
