...
function prodCard(p){
  ...
  return '<div class="prod-card" onclick="openProduct(' + pidSafe + ')">'
    ...
    + '<button class="prod-wish" onclick="event.stopPropagation();handleWish(' + pidSafe + ',this)" title="Save" style="color:var(--navy)"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg></button>'
    + '</div>'
    + '<div class="prod-body">'
...
async function renderDashboard(){
  ...
  document.getElementById('dashRfqList').innerHTML = rfqs?.length
    ? rfqs.slice(0,3).map(r=>{var st=(r.status||'new').toLowerCase();var stColor=st==='new'?'var(--gold)':st==='replied'?'#059669':st==='viewed'?'#4f8ef7':'#9ca3af';var reply=(r.reply||'').trim();return `<div style="padding:10px 0;border-bottom:1px solid var(--border)"><div style="display:flex;justify-content:space-between;align-items:center;gap:8px"><div style="font-weight:600;color:var(--text);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1">${r.project_name||'Untitled Project'}</div><span style="font-size:8px;font-weight:800;color:${stColor};border:1px solid ${stColor};padding:2px 6px;border-radius:100px;text-transform:uppercase;letter-spacing:.5px">${st}</span></div><div style="font-size:10px;color:var(--muted);margin-top:2px">${r.brand_name||r.project_type||'—'} · ${new Date(r.created_at).toLocaleDateString('en-GB')}</div>${reply?`<div style="font-size:11px;color:var(--text);margin-top:6px;padding:8px 10px;background:#f0fdf4;border-left:3px solid #059669;border-radius:6px;line-height:1.5"><div style="font-size:9px;font-weight:800;color:#059669;text-transform:uppercase;letter-spacing:.5px;margin-bottom:2px">Reply from ${r.brand_name||'brand'}</div>${reply.length>140?reply.substring(0,140)+'…':reply}</div>`:''}</div>`;}).join('')
  ...
  document.getElementById('dashSampleList').innerHTML = samples?.length
    ? samples.slice(0,3).map(r=>{var st=(r.status||'new').toLowerCase();var stColor=st==='new'?'var(--gold)':st==='replied'?'#059669':st==='viewed'?'#4f8ef7':'#9ca3af';var reply=(r.reply||'').trim();return `<div style="padding:10px 0;border-bottom:1px solid var(--border)"><div style="display:flex;justify-content:space-between;align-items:center;gap:8px"><div style="font-weight:600;color:var(--text);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1">${r.product_name||'Product Sample'}</div><span style="font-size:8px;font-weight:800;color:${stColor};border:1px solid ${stColor};padding:2px 6px;border-radius:100px;text-transform:uppercase;letter-spacing:.5px">${st}</span></div><div style="font-size:10px;color:var(--muted);margin-top:2px">${r.company||'—'} · ${new Date(r.created_at).toLocaleDateString('en-GB')}</div>${reply?`<div style="font-size:11px;color:var(--text);margin-top:6px;padding:8px 10px;background:#f0fdf4;border-left:3px solid #059669;border-radius:6px;line-height:1.5"><div style="font-size:9px;font-weight:800;color:#059669;text-transform:uppercase;letter-spacing:.5px;margin-bottom:2px">Reply</div>${reply.length>140?reply.substring(0,140)+'…':reply}</div>`:''}</div>`;}).join('')
...
function inboxCardHtml(r, kind){
  ...
  const statusColor = status==='new' ? 'var(--gold)' : status==='replied' ? '#059669' : status==='viewed' ? '#4f8ef7' : '#9ca3af';
  ...
  return '<div style="background:white;border:1px solid var(--border);border-radius:10px;padding:12px 14px 12px 18px;display:flex;align-items:center;gap:12px;position:relative;cursor:pointer;transition:border-color .15s,box-shadow .15s" '
    ...
    + '<div style="display:flex;align-items:center;gap:8px;flex-shrink:0">'
      + '<span style="font-size:9px;font-weight:800;color:'+statusColor+';background:transparent;border:1px solid '+statusColor+';padding:3px 8px;border-radius:100px;text-transform:uppercase;letter-spacing:.6px">' + status + '</span>'
...
// ---------- INBOX REPLY MODAL (shared by RFQ + sample) ----------
window._irRow = null;
window._irKind = null;
...
async function openInboxReply(kind, id){
  ...
  document.getElementById('irSubmitter').textContent = (submitter||'\u2014') + (company?' \u00b7 '+company:'') + (email?' \u00b7 '+email:'') + (phone?' \u00b7 '+phone:'');

  const message = kind==='rfq' ? (row.message||'') : (row.notes||'');
  const detail = kind==='rfq'
    ? '<div style="display:flex;flex-direction:column;gap:8px;font-size:12px;color:var(--text)">'
      + '<div><strong style="color:var(--navy2)">Project:</strong> ' + (row.project_name||'\u2014') + '</div>'
      + '<div><strong style="color:var(--navy2)">Type:</strong> ' + (row.project_type||'\u2014') + '</div>'
      + '<div><strong style="color:var(--navy2)">Location:</strong> ' + (row.project_location||'\u2014') + '</div>'
      + '<div><strong style="color:var(--navy2)">Required by:</strong> ' + (row.required_by||'\u2014') + '</div>'
    + '</div>'
    : '<div style="display:flex;flex-direction:column;gap:8px;font-size:12px;color:var(--text)">'
      + '<div><strong style="color:var(--navy2)">Product:</strong> ' + (row.product_name||'\u2014') + '</div>'
      + '<div><strong style="color:var(--navy2)">Delivery:</strong> ' + (row.address||'\u2014') + '</div>'
    + '</div>';
  document.getElementById('irBody').innerHTML = detail
    + (message ? '<div style="font-size:13px;color:var(--text);line-height:1.7;background:var(--off);padding:14px 16px;border-radius:10px;white-space:pre-wrap;margin-top:12px">' + message + '</div>' : '');

  document.getElementById('ir-reply').value = row.reply || '';
  // Set status dropdown
  const dd = document.getElementById('ir-status-dd');
  if(dd){
    const cur = (row.status || 'new').toLowerCase();
    const lbl = dd.querySelector('.form-dd-label');
    if(lbl){ lbl.textContent = cur; lbl.classList.remove('form-dd-placeholder'); }
    document.getElementById('ir-status').value = cur;
    dd.querySelectorAll('.form-dd-opt').forEach(function(o){ o.classList.toggle('selected', o.textContent.trim() === cur); });
  }
...
async function saveInboxReply(){
  ...
  if(!row || !kind) return;
  const reply = document.getElementById('ir-reply').value.trim();
  const status = (document.getElementById('ir-status').value || 'new').toLowerCase();
  const msg = document.getElementById('irMsg');
  const showMsg = function(t, err){ if(!msg) return; msg.textContent = t; msg.style.display='block'; msg.style.background=err?'#fef2f2':'#d1fae5'; msg.style.color=err?'#dc2626':'#065f46'; };
  const payload = { status: status, reply: reply || null, replied_at: reply ? new Date().toISOString() : null };
  ...
  try {
    ...
    setTimeout(function(){
      closeInboxReply();
      if(kind === 'rfq') renderBrandInbox();
      else renderBrandSampleInbox();
    }, 600);
  } catch(e){
    console.error('Reply save failed', e);
    showMsg('\u26a0 ' + (e.message || 'Could not save.'), true);
  }
  if(btn){ btn.disabled = false; btn.textContent = 'Save reply & status'; }
}
...
async function refreshAllSavedBody(){
  ...
  document.getElementById('vcBody').innerHTML = '<div class="prod-grid">' + saved.map(function(p){
    ...
    var inColTag = p.collection_id && colNameById[p.collection_id]
      ? '<span class="prod-cat-tag" style="background:rgba(0,51,102,.85)">' + colNameById[p.collection_id] + '</span>'
      : '<span class="prod-cat-tag">Saved</span>';
...
function srClearForm(){
  ...
  if(wrap) wrap.style.display = 'none';
  srRenderProductChip();
  ['srFirst','srLast','srEmail','srPhone','srCompany','srDeliveryAddress','srNotes','srRole'].forEach(function(id){
    const el = document.getElementById(id); if(el) el.value='';
  });
...
async function submitSR(){
  ...
  const addr    = (document.getElementById('srDeliveryAddress')||{}).value || '';
  ...
  if(!first.trim() || !last.trim() || !email.trim() || !phone.trim() || !company.trim() || !role || !addr.trim() || !notes.trim()){
...
(function(){
  ...
  window.showPage = function(p){
    ...
    if(currentUser){
      if(p === 'rfq'){
        setTimeout(function(){
          var f = document.getElementById('rfqEmail'); if(f && !f.value) f.value = currentUser.email || '';
        }, 60);
      }
      if(p === 'samplerequest'){
        setTimeout(function(){
          var f = document.getElementById('srEmail'); if(f && !f.value) f.value = currentUser.email || '';
        }, 60);
      }
    }
    return r;
