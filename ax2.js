...
function renderProductModal(p){
  ...
  if(el){ var _bn=(p.brand||''); el.textContent=_bn.toUpperCase(); if(_bn){ el.style.cursor='pointer'; el.style.textDecoration='underline'; el.style.textUnderlineOffset='2px'; el.title='View '+_bn+' profile'; el.onclick=function(){ closeModal(); openBrandByName(_bn); }; } else { el.style.cursor=''; el.onclick=null; } }
