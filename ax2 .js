...
async function renderManufacturers(){
  ...
  if(!grid) return;
  if(!brands.length){
    grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:60px"><div style="font-size:40px;margin-bottom:12px">\uD83C\uDFED</div><div style="font-size:15px;font-weight:700;color:var(--text);margin-bottom:6px">Manufacturers joining soon</div><div style="font-size:12px;color:var(--muted)">European brands are being onboarded to Archspex</div></div>';
    return;
  }
...
async function openBrandProfile(id){
  ...
  if(!overlay){
    ...
    overlay.id = 'brandProfileOverlay';
  ...
  var products_html = (prods||[]).length ? (prods||[]).map(function(p){
    return '<div onclick="overlay.remove();openProduct(\'db_\'+'+p.id+')" style="display:flex;align-items:center;gap:12px;padding:10px;border:1px solid var(--border);border-radius:8px;cursor:pointer;transition:background .15s" onmouseover="this.style.background=\'var(--off)\'" onmouseout="this.style.background=\'\'"> '
      + '<img src="'+(p.image_url||'')+'" style="width:56px;height:56px;object-fit:cover;border-radius:6px;flex-shrink:0" onerror="this.src=\'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=100\'">'
      + '<div><div style="font-size:12px;font-weight:700;color:var(--text)">'+(p.name||'')+'</div>'
      + '<div style="font-size:10px;color:var(--muted)">'+(p.category||'')+(p.meta?' \xb7 '+p.meta:'')+'</div></div>'
      + '<div style="margin-left:auto;font-size:10px;font-weight:700;color:var(--navy)">View \u2192</div></div>';
  }).join('') : '<div style="text-align:center;padding:24px;color:var(--muted);font-size:12px">No products listed yet</div>';
  ...
  overlay.innerHTML = '<div style="background:white;border-radius:8px;max-width:680px;width:100%;max-height:88vh;overflow-y:auto;position:relative">'
    + '<div style="background:var(--navy);padding:32px;border-radius:8px 8px 0 0">'
    + '<button onclick="document.getElementById(\'brandProfileOverlay\').remove()" style="position:absolute;top:16px;right:16px;background:rgba(255,255,255,.15);border:none;color:white;width:32px;height:32px;border-radius:50%;font-size:16px;cursor:pointer">\u00d7</button>'
    + '<div style="width:64px;height:64px;border-radius:12px;background:white;display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:900;color:var(--navy);margin-bottom:16px">'+(brand.name||'').substring(0,2).toUpperCase()+'</div>'
    + '<div style="font-family:Fraunces,serif;font-size:28px;font-weight:300;color:white;margin-bottom:4px">'+(brand.name||'')+'</div>'
    + '<div style="font-size:12px;color:rgba(255,255,255,.6)">'+(brand.city||'')+(brand.city&&brand.country?', ':'')+(brand.country||'')+(brand.founded?' \xb7 Est. '+brand.founded:'')+'</div>'
    + '</div>'
    + '<div style="padding:28px">'
    + '<div style="font-size:13px;color:var(--muted);line-height:1.7;margin-bottom:24px">'+(brand.description||'')+'</div>'
    + '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:24px">'
    + '<div style="text-align:center;padding:16px;background:var(--off);border-radius:8px"><div style="font-size:18px;font-weight:800;color:var(--navy)">'+(brand.employees||'—')+'</div><div style="font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:.5px">Employees</div></div>'
    + '<div style="text-align:center;padding:16px;background:var(--off);border-radius:8px"><div style="font-size:18px;font-weight:800;color:var(--navy)">'+(brand.founded||'—')+'</div><div style="font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:.5px">Founded</div></div>'
    + '<div style="text-align:center;padding:16px;background:var(--off);border-radius:8px"><div style="font-size:18px;font-weight:800;color:var(--navy)">'+(prods||[]).length+'</div><div style="font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:.5px">Products</div></div>'
    + '</div>'
    + '<div style="font-size:10px;font-weight:800;color:var(--muted);text-transform:uppercase;letter-spacing:1px;margin-bottom:12px">Products on Archspex</div>'
    + '<div style="display:flex;flex-direction:column;gap:8px;margin-bottom:24px">' + products_html + '</div>'
    + (brand.website ? '<a href="'+brand.website+'" target="_blank" style="display:block;text-align:center;background:var(--navy);color:white;border-radius:6px;padding:12px;font-size:12px;font-weight:700;text-decoration:none">Visit Website \u2192</a>' : '')
    + '</div></div>';
}
